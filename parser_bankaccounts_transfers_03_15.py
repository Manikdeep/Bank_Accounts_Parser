...
class Bank:
    ...
    def check_amount(self, content):
        ...
        for i in re.finditer(r"\d{1,3}(?:,\d{3})*(?:\.\d+)+", content):
            ...
            if "+" in check_trans or "-" in check_trans:
                continue
            else:
                result.add(i.group())
        ...
        if num_banks == 1:
            for i in range(0, max(num_banks,num_acc_type,num_amount)):
                ...
                if num_amount > 0:
                    ...
                    amt = ""
                writer.writerow([photo_id, date, bank[0], acc_type, "", amt])
            return 
        else:
            for i in range(0, max(num_banks,num_acc_type,num_amount)):
                if num_acc_type > 0:
                ...
                if num_banks > 0:
                    ...
                    bank_name = ""
                writer.writerow([photo_id, date, bank_name, acc_type, "", amt])

if __name__ == '__main__':
    folder_textdoc_path = sys.argv[1]
    textdoc_paths = base_parser.get_textdoc_paths(folder_textdoc_path)
    headerList = ['Pic Id', 'Date', 'Bank Name', 'Account Type', 'Account Number', 'Amount', 'Available Amount']
